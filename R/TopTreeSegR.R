...
#' Build Ultra-Fast Gradient Flow (Updated with Paper Method)
#' 
#' @keywords internal
build_gradient_flow_ultra_paper <- function(morse_complex, vertices, max_distance = 2.0, grid_size = 5.0) {
  ...
  # PAPER'S METHOD: Build minima connectivity from 1-saddles
  ...
  list(
    ...
    ascending_regions = as.integer(ascending_regions),
    gradient_network = gradient_network,
    minima_connectivity = minima_connectivity  # Now using paper's method
  )
}
